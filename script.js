...
function atualizarTela() {
  ...
  transacoes.forEach((item, index) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn-acao edit" onclick="editar(${index})">✏️</button>
        <button class="btn-acao delete" onclick="deletar(${index})">🗑️</button>
      </td>
    `;
...
function atualizarGrafico(entradas = 0, saidas = 0) {
  ...
  const corTexto = document.body.classList.contains('dark-theme')
    ? '#e1e1e6'
    : '#363f5f';
...
// ✔ PREVIEW DO ÍCONE (SOMENTE VISUAL ANTES DE INSTALAR)
